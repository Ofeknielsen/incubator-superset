...
class DashboardTestCase(SupersetTestCase):
    ...
    def save_dashboard_via_view(self, dashboard_id: Union[str, int], dashboard_data: Dict) -> Response:
        save_dash_url = SAVE_DASHBOARD_URL_FORMAT.format(dashboard_id)
        return self.get_resp(
            save_dash_url, data=dict(data=json.dumps(dashboard_data))
        )

    def save_dashboard(self, dashboard_id: Union[str, int], dashboard_data: Dict) -> Response:
    ...
    def save_dash_basic_case(self, username=ADMIN_USERNAME):
        ...
        # act
        save_dash_response = self.save_dashboard_via_view(dashboard_to_save.id, data_after_change)
