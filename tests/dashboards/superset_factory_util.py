...
from tests.dashboards.dashboard_test_utils import random_slug, random_title, random_str, is_dashboard_level_access_enabled
...
def create_dashboard_to_db(dashboard_title: Optional[str] = None,
...
def create_dashboard(dashboard_title: Optional[str] = None, slug: Optional[str] = None,
                     published: bool = False, owners: Optional[List[User]] = None,
                     slices: Optional[List[Slice]] = None, css: str = "",
                     json_metadata: str = "", position_json: str = "") -> Dashboard:
...
def create_slice_to_db(name: Optional[str] = None,
                       datasource_id: Optional[int] = None,
                       owners: Optional[List[User]] = None) -> Slice:
...
def create_slice(datasource_id, name, owners):
    ...
    owners = owners or []
    datasource_id = datasource_id or create_datasource_table_to_db(name=name + "_table").id
    return Slice(slice_name=name, datasource_id=datasource_id, owners=owners, datasource_type="table")


def create_datasource_table_to_db(name: Optional[str] = None,
                                  db_id: Optional[int] = None,
                                  owners: Optional[List[User]] = None) -> SqlaTable:
...
def create_datasource_table(name: Optional[str] = None,
                            db_id: Optional[int] = None,
                            owners: Optional[List[User]] = None) -> SqlaTable:
    name = name or random_str()
    owners = owners or []
...
def delete_all_inserted_dashboards():
    try:
        dashboards_to_delete: List[Dashboard] = session.query(Dashboard) \
            .filter(Dashboard.id.in_(inserted_dashboards_ids)).all()
...
def delete_all_inserted_slices():
    try:
        slices_to_delete: List[Slice] = session.query(Slice) \
            .filter(Slice.id.in_(inserted_slices_ids)).all()
...
def delete_slice_users_associations(slice_: Slice) -> None:
    session.execute(
        slice_user.delete().where(
            slice_user.c.slice_id == slice_.id
        )
    )
...
def delete_all_inserted_tables():
    try:
        tables_to_delete: List[SqlaTable] = session.query(SqlaTable) \
            .filter(SqlaTable.id.in_(inserted_sqltables_ids)).all()
...
def delete_table_users_associations(table: SqlaTable) -> None:
    session.execute(
        sqlatable_user.delete().where(
            sqlatable_user.c.table_id == table.id
        )
    )
...
def delete_all_inserted_dbs():
    try:
        dbs_to_delete: List[Database] = session.query(Database) \
            .filter(Database.id.in_(inserted_databases_ids)).all()
...
def delete_all_inserted_models(model: Any, ids: List[int], delete_model: Callable):
    try:
        models_to_delete: List[Model] = session.query(model) \
            .filter_by(model.id.in_(ids).all())
