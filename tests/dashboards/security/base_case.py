...
class BaseTestDashboardSecurity(DashboardTestCase):
    ...
    def assert_dashboard_view_response(self, response: Response, dashboard_to_access: Dashboard) -> None:
        self.assert200(response)
        assert escape(dashboard_to_access.dashboard_title) in response.data.decode("utf-8")

    def assert_dashboard_api_response(self, response: Response, dashboard_to_access: Dashboard) -> None:
        self.assert200(response)
        self.assertEqual(response.json["id"], dashboard_to_access.id)

    def assert_dashboards_list_view_response(self, response: Response, expected_counts: int,
                                             expected_dashboards: Optional[List[Dashboard]] = None,
                                             not_expected_dashboards: Optional[List[Dashboard]] = None) -> None:
        self.assert200(response)
        response_html = response.data.decode("utf-8")
        if expected_counts == 0:
            assert "No records found" in response_html
        else:
            assert DASHBOARD_COUNT_IN_DASHBOARDS_LIST_VIEW_FORMAT.format(count=str(expected_counts)) in response_html
    ...
    def assert_dashboards_api_response(self, response: Response, expected_counts: int,
                                       expected_dashboards: Optional[List[Dashboard]] = None,
                                       not_expected_dashboards: Optional[List[Dashboard]] = None) -> None:
        self.assert200(response)
        response_data = response.json
        self.assertEqual(response_data["count"], expected_counts)
        response_dashboards_url = set(map(lambda dash: dash["url"], response_data["result"]))
        expected_dashboards = expected_dashboards or []
        for dashboard in expected_dashboards:
